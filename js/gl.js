...
var Gl = {
  ...
  getDisplay: function() {
    return [this.displayWidth, this.displayHeight, this.displayDepth];
  },

  // functions
  createShader: function(shaderType, shaderCode) {
    var shader = this.gl.createShader(
      shaderType == ShaderKind.VERTEX
  ...
  createBuffer: function(attribute) {
    ...
    this.getAttributes().forEach(localAttribute => {
      if (
        attribute.equals(localAttribute) &&
    ...
    if (foundAttribute != null) return foundAttribute.getBuffer();

    var buffer = this.gl.createBuffer();
    this.gl.bindBuffer(attribute.getTarget(), buffer);
    this.gl.bufferData(
      attribute.getTarget(),
      attribute.getSrcData(),
      this.gl.STATIC_DRAW
    );
  ...
  setTexture: function(newTexture, newImage) {
    ...
    image.addEventListener("load", function() {
      ...
      image.tag.texImage2D(
        image.tag.TEXTURE_2D,
        0,
        image.tag.RGBA,
        image.tag.RGBA,
        image.tag.UNSIGNED_BYTE,
        image
      );
      ...
      if (isPowerOf2(image.width) && isPowerOf2(image.height)) {
        ...
        image.tag.texParameteri(
          image.tag.TEXTURE_2D,
          image.tag.TEXTURE_WRAP_S,
          image.tag.CLAMP_TO_EDGE
        );
        image.tag.texParameteri(
          image.tag.TEXTURE_2D,
          image.tag.TEXTURE_WRAP_T,
          image.tag.CLAMP_TO_EDGE
        );
        image.tag.texParameteri(
          image.tag.TEXTURE_2D,
          image.tag.TEXTURE_MIN_FILTER,
          image.tag.LINEAR
        );
      }
    });
