...
var VertexShader = function() {
  ...
  this.getAttributeNormalsName = function() {
    return this.attributeNormalsName;
  };
  this.setAttributeNormalsName = function(newAttributeNormalsName) {
    this.attributeNormalsName = newAttributeNormalsName;
  };

  this.getAttributeColorName = function() {
    return this.attributeColorName;
  };
...
var VertexShaderService = function(item) {
  ...
  this.buildVarying = function() {
    ...
    if (this.getAttributeColorName() != null) {
      text +=
        "varying vec4 " +
        this.getAttributeColorName().replace("a_", "v_") +
        ";\n";
    }
    if (this.getAttributeNormalsName() != null) {
      text +=
        "varying vec3 " +
        this.getAttributeNormalsName().replace("a_", "v_") +
        ";\n";
    }
    if (this.attributeUVCoordsName != null) {
      text +=
        "varying vec2 " +
        this.attributeUVCoordsName.replace("a_", "v_") +
        ";\n";
    }
    if (this.uniformLightPosition != null) {
  ...
  this.buildInnerMain = function() {
    ...
    if (
      this.getAttributeNormalsName() != null &&
      this.uniformObjectMatrixName != null
    ) {
      text +=
        this.getAttributeNormalsName().replace("a_", "v_") +
  ...
  uniforms.forEach(uniform => {
    ...
    if (
      uniform.getKind() == UniformKind.CAMERAMATRIX ||
      uniform.getKind() == UniformKind.LOOKATMATRIX
    ) {
